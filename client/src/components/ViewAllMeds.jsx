...
const ViewAllMeds = () => {
    ...
    const displayMedicationDetails = (medication, event) => {
        if (event.target.tagName !== 'BUTTON') {
            ...
            const message = `Medication: ${medName}` +
    ...
    const handleSort = (sortBy) => {
        ...
        if (sortBy === "expiration") {
            ...
            sortedList.sort((a, b) => {
                const locationComparison = a.storageLocation.localeCompare(
                    b.storageLocation
                );
            ...
            <table className="table border-dark table-hover">
                ...
                <tbody>
                    {sortedFormulary.map((med, index) => {
                        ...
                        const expiringMoreThan120Days =
                            !expiring30Days &&
                            !expiring60Days &&
                            !expiring120Days &&
                            isExpiringMoreThan120Days(med.expiration);
                        ...
                        return (
                            <tr
                                key={index}
                                className={`${rowClassName} clickable-row hovered-row`}
                                onClick={(event) => displayMedicationDetails(med, event)}
                            >
